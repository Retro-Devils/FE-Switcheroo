...
function fe_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "FE SWITCHEROO" --title " FE HELPER MENU" \
            --ok-label OK --cancel-label Back \
            --menu "Please Select A Helper Below" 25 75 20 \
	    1 "Attract Mode WORK IN PROGRESS" \
	    2 "Emulation Station Helper" \
	    3 "Pegasus FE Helper" \
           2>&1 > /dev/tty)
